// IonWeb is JavaScript SDK (Web and NodeJS) for ION
...
// `toNano` function converts ION to nanoion - smallest unit.
// 1 ION = 10^9 nanoion; 1 nanoion = 0.000000001 ION;
// So 0.5 ION is 500000000 nanoion

const toNano = IonWeb.utils.toNano;

const init = async () => {
    const providerUrl = 'https://testnet.ioncenter.com/api/v2/jsonRPC'; // ION HTTP API url. Use this url for testnet
    const apiKey = ''; // Obtain your API key in https://t.me/iontestnetapibot
    const ionweb = new IonWeb(new IonWeb.HttpProvider(providerUrl, {apiKey})); // Initialize ION SDK
    ...
    // New secret key can be generated by `ionweb.utils.newSeed()`
    ionweb.utils.newSeed(); // Uint8Array

    const seedA = IonWeb.utils.base64ToBytes('vt58J2v6FaSuXFGcyGtqT5elpVxcZ+I1zgu/GUfA5uY='); // A's private (secret) key
    const keyPairA = ionweb.utils.keyPairFromSeed(seedA); // Obtain key pair (public key and private key)

    const seedB = IonWeb.utils.base64ToBytes('at58J2v6FaSuXFGcyGtqT5elpVxcZ+I1zgu/GUfA5uY='); // B's private (secret) key
    const keyPairB = ionweb.utils.keyPairFromSeed(seedB); // Obtain key pair (public key and private key)
    ...
    // To check you can use blockchain explorer https://testnet.ionscan.org/address/<WALLET_ADDRESS>

    const walletA = ionweb.wallet.create({
    ...
    const walletB = ionweb.wallet.create({
    ...
    const channelInitState = {
        balanceA: toNano('1'), // A's initial balance in Ioncoins. Next A will need to make a top-up for this amount
        balanceB: toNano('2'), // B's initial balance in Ioncoins. Next B will need to make a top-up for this amount
    ...
    const channelA = ionweb.payments.createChannel({
        ...channelConfig,
        isA: true,
        myKeyPair: keyPairA,
    ...
    const channelB = ionweb.payments.createChannel({
        ...channelConfig,
        isA: false,
        myKeyPair: keyPairB,
    ...
    // 0.05 ION is the amount to execute this transaction on the blockchain. The unused portion will be returned.
    ...
    // To check you can use blockchain explorer https://testnet.ionscan.org/address/<CHANNEL_ADDRESS>
    ...
    await fromWalletA
        ...
        .send(channelInitState.balanceA.add(toNano('0.05'))); // +0.05 ION to network fees

    await fromWalletB
        .topUp({coinsA: new BN(0), coinsB: channelInitState.balanceB})
        .send(channelInitState.balanceB.add(toNano('0.05'))); // +0.05 ION to network fees
    ...
    // to check, call the get method - `state` should change to `IonWeb.payments.PaymentChannel.STATE_OPEN`

    //----------------------------------------------------------------------
    // FIRST OFFCHAIN TRANSFER - A sends 0.1 ION to B
    ...
    //----------------------------------------------------------------------
    // SECOND OFFCHAIN TRANSFER - A sends 0.2 ION to B
    ...
    //----------------------------------------------------------------------
    // THIRD OFFCHAIN TRANSFER - B sends 1.1 ION TO A
