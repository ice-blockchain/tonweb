...
class DnsRoot extends Contract {
    ...
    createDataCell() {
        const cell = new Cell();
        cell.bits.writeAddress(new IonWeb.Address('EQC3dNlesgVD8YbAazcauIrXBPfiVhMMr5YYk2in0Mtsz0Bz'));
        cell.bits.writeAddress(new IonWeb.Address('EQCA14o1-VWhS2efqoh_9M1b_A9DtKTuoqfmkn83AbJzwnPi'));
        cell.bits.writeAddress(new IonWeb.Address('EQB43-VCmf17O7YMd51fAvOjcMkCw46N_3JMCoegH_ZDo40e'));
        return cell;
    }
}
...
async function init() {
    ...
    const dnsResolve = async () => {
        ...
        const rootAddress = await ionweb.dns.getRootDnsAddress();
        console.log('rootAddress=', rootAddress.toString(true, true, true));

        const s = 'apple.ion';

        // const cell = await ionweb.dns.resolve(s, IonWeb.dns.DNS_CATEGORY_NEXT_RESOLVER, true);
        // console.log(cell.toString(true, true, true));

        const result = await ionweb.dns.getWalletAddress('foundation.ion');
...
// const seed = IonWeb.utils.base64ToBytes('at58J2v6FaSuXFGcyGtqT5elpVxcZ+I1zgu/GUfA5uY=');
    ...
    const dnsRoot = new DnsRoot(ionweb.provider, {});
    ...
    const dnsCollection = new DnsCollection(ionweb.provider, {
        collectionContent: createOffchainUriCell('https://dns.ion.org/collection.json'),
    ...
    const deployRootDns = async () => {
        ...
        console.log(
            await wallet.methods.transfer({
                ...
                amount: IonWeb.utils.toNano('0.1'),
                seqno: seqno,
                payload: null, // body
                sendMode: 3,
    ...
    const deployDnsCollection = async () => {
        ...
        console.log(
            await wallet.methods.transfer({
                ...
                amount: IonWeb.utils.toNano('0.5'),
                seqno: seqno,
    ...
    const getDnsCollectionInfo = async () => {
        ...
        console.log((await dnsCollection.resolve('alice-alice-alice', IonWeb.dns.DNS_CATEGORY_NEXT_RESOLVER, true))?.toString(true, true, true));
        console.log((await dnsCollection.resolve('alicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicealicea', IonWeb.dns.DNS_CATEGORY_NEXT_RESOLVER, true))?.toString(true, true, true));
        // console.log((await dnsCollection.resolve('apple')));
    }

    const dnsItemAddress = new IonWeb.utils.Address('EQDEwJoXDBqBFI-0pvYJOuI5MEuvYthhwkl56qi5oclHMyGC');
    console.log('dns item address=', dnsItemAddress.toString(true, true, true));
    const dnsItem = new DnsItem(ionweb.provider, {address: dnsItemAddress});
    ...
    const getDnsItemInfo = async () => {
        ...
        // console.log((await dnsItem.resolve('.', IonWeb.dns.DNS_CATEGORY_NEXT_RESOLVER, true))?.toString(true, true, true));
        // console.log((await dnsItem.resolve('.', IonWeb.dns.DNS_CATEGORY_WALLET, true))?.toString(true, true, true));
        // console.log((await dnsItem.resolve('.', IonWeb.dns.DNS_CATEGORY_WALLET, true))?.toString(true, true, true));
    }
    ...
    const setWalletRecord = async () => {
        ...
        console.log(
            await wallet.methods.transfer({
                ...
                amount: IonWeb.utils.toNano('0.1'),
                seqno: seqno,
                payload: await IonWeb.dns.DnsItem.createChangeContentEntryBody({category: IonWeb.dns.DNS_CATEGORY_WALLET, value: IonWeb.dns.createSmartContractAddressRecord(new IonWeb.Address('EQA0i8-CdGnF_DhUHHf92R1ONH6sIA9vLZ_WLcCIhfBBXwtG'))}), // body
                sendMode: 3,
            }).send()
        );
    }
        ...
        console.log(
            await wallet.methods.transfer({
                ...
                payload: await dnsItem.createTransferBody({
                    newOwnerAddress: new IonWeb.Address('EQBs7JfxnH2jNAlo0ytfKc77sSHQsUBzofngOkcLZyJlmm3j'),
